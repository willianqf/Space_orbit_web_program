...
class PveRoom(GameRoom):
    ...
    def _update_game_logic(self, dt):
        ...
        toremove_proj = []; toremove_obs = []; toremove_npc = []
            ...
            if proj['tipo'].startswith('player'):
                for npc in self.npcs:
                    ...
                    if (npc['x'] - proj['x'])**2 + (npc['y'] - proj['y'])**2 < (raio_npc + 5)**2:
                        npc['hp'] -= proj['dano']; npc['ultimo_hit_tempo'] = self.agora_ms; hit = True
                        if npc['hp'] <= 0:
...
class PvpRoom(GameRoom):
    ...
    def update(self, dt_multiplier=1.0):
        ...
        if self.state == "WAITING":
            ...
            if len(self.clients) < 4: self.state = "WAITING"
            ...
            survivors = [p for p in self.players.values() if p['hp'] > 0]
            if len(survivors) <= 1:
    ...
    def _update_pvp_physics(self, dt):
        ...
        for proj in self.projectiles:
            ...
            if not (0 <= proj['x'] <= pvp_s.MAP_WIDTH and 0 <= proj['y'] <= pvp_s.MAP_HEIGHT): toremove_proj.append(proj); continue
            if damage_enabled:
                for target in living_players:
                    if target['nome'] == proj['owner_nome']: continue
                    if (target['x'] - proj['x'])**2 + (target['y'] - proj['y'])**2 < COLISAO_JOGADOR_PROJ_DIST_SQ:
        ...
        for p in self.players.values():
            if p['hp'] > 0:
                p_data = {
                    ...
                    "nv_motor": p['nivel_motor'], "nv_dano": p['nivel_dano'], 
                    "nv_hp": p['nivel_max_vida'], "nv_escudo": p['nivel_escudo'], 
                    "nv_aux": p['nivel_aux']
                }
                
                # --- ENVIO DA FLAG NO JSON PVP ---
                if p.get('shield_hit') and self.agora_ms - p['shield_hit']['time'] < 200:
...
async def handler(websocket):
    ...
    try:
        ...
        if not current_room: await websocket.close(1008, "Salas cheias ou em progresso"); return

        spawn_x, spawn_y = 0, 0
        if mode == "PVP":
            spawn_x, spawn_y = pvp_s.SPAWN_LOBBY.x, pvp_s.SPAWN_LOBBY.y
            upgrade_pts = 10; map_w, map_h = pvp_s.MAP_WIDTH, pvp_s.MAP_HEIGHT
        else:
            spawn_x, spawn_y = server_calcular_posicao_spawn([], s.MAP_WIDTH, s.MAP_HEIGHT)
            upgrade_pts = 0; map_w, map_h = s.MAP_WIDTH, s.MAP_HEIGHT
        ...
        await websocket.send(json.dumps({
            "type": "WELCOME", "id": p_state['nome'], "x": spawn_x, "y": spawn_y, 
            "mode": mode, "map_width": map_w, "map_height": map_h
        }))
            ...
            try:
                ...
                if cmd["type"] == "INPUT":
                    ...
                    for p in current_room.players.values():
                        if p['nome'] != p_state['nome']: candidatos.append(p)
