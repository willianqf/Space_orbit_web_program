...
# ==================================================================================
# CLASSE VECTOR2 (SUBSTITUI PYGAME)
# ==================================================================================
class Vector2:
    """Substituto leve para pygame.math.Vector2 - sem dependências externas"""
    __slots__ = ('x', 'y')  # Otimização de memória
    ...
    def rotate(self, angle_degrees):
        """Rotaciona o vetor por um ângulo em graus"""
...
# CONSTANTES KAMIKAZE
KAMIKAZE_DIST_DETONACAO_SQ = s.KAMIKAZE_DIST_DETONACAO**2
KAMIKAZE_RAIO_EXPLOSAO_SQ = s. KAMIKAZE_RAIO_EXPLOSAO**2
...
def _rotate_vector(x, y, angle_degrees):
    """Rotaciona um vetor (x, y) por um ângulo em graus"""
    rad = math.radians(angle_degrees)
    cos_a = math.cos(rad)
    sin_a = math.sin(rad)
...
def calc_hit_angle_rad(target_x, target_y, attacker_x, attacker_y):
    """Calcula o ângulo de impacto em radianos"""
...
def server_calcular_posicao_spawn(pos_referencia_lista, map_width, map_height):
    """Calcula uma posição de spawn longe de outras entidades"""
    margem = 100
    dist_min_sq = 600**2
    
    for _ in range(20):
        x = random.uniform(margem, map_width - margem)
        y = random.uniform(margem, map_height - margem)
        
        longe = True
        if pos_referencia_lista:
            for px, py in pos_referencia_lista:
                if (x - px)**2 + (y - py)**2 < dist_min_sq:
                    longe = False
                    break
    ...
    return (
        float(random.uniform(margem, map_width - margem)),
        float(random.uniform(margem, map_height - margem))
    )


# ==================================================================================
# 3. SISTEMA DE PONTOS E UPGRADES
# ==================================================================================

def server_ganhar_pontos(player_state, quantidade):
    """Adiciona pontos ao jogador e verifica upgrades disponíveis"""
    if quantidade <= 0:
        return
    ...
    if limiar <= 0:
        limiar = PONTOS_LIMIARES_PARA_UPGRADE[0] if PONTOS_LIMIARES_PARA_UPGRADE else 100
        player_state['_limiar_pontos_atual'] = limiar
    
    # Limite de iterações para segurança
    for _ in range(50):
        if player_state['_pontos_acumulados_para_upgrade'] < player_state['_limiar_pontos_atual']:
            break
        
        player_state['pontos_upgrade_disponiveis'] += 1
        player_state['_pontos_acumulados_para_upgrade'] -= player_state['_limiar_pontos_atual']
        
        # Verifica se deve aumentar o limiar
        pontos_totais = player_state['pontos']
        indice = player_state. get('_indice_limiar', 0)
    ...
    if tipo_upgrade == "motor":
        ...
        if num < MAX_AUXILIARES:
            ...
            if player_state['pontos_upgrade_disponiveis'] >= custo_aux and pode_comprar(custo_aux):
                player_state['pontos_upgrade_disponiveis'] -= custo_aux
                player_state['total_upgrades_feitos'] += custo_aux
        ...
        if player_state['nivel_max_vida'] < len(VIDA_POR_NIVEL) - 1 and pode_comprar(custo_padrao):
            ...
            nova_vida = float(VIDA_POR_NIVEL[player_state['nivel_max_vida']])
            diff = nova_vida - player_state['max_hp']
            player_state['max_hp'] = nova_vida
            player_state['hp'] += diff
...
def update_player_logic(player_state, lista_alvos_busca, agora_ms, map_width, map_height, dt=1.0):
    """Atualiza a lógica do jogador (movimento, rotação, tiros)"""
    
    # Lógica do Propulsor Energético
    if player_state. get('propulsor_ativo', False):
        if agora_ms > player_state.get('fim_propulsor', 0):
            player_state['propulsor_ativo'] = False
    ...
    # Rotação baseada em alvo
    if player_state['alvo_lock']:
        alvo = None
        for e in lista_alvos_busca:
            if e. get('id', e. get('nome')) == player_state['alvo_lock']:
                alvo = e
                break
        ...
        if alvo and alvo.get('hp', 0) > 0 and not alvo.get('propulsor_ativo', False):
            ...
            if vec_x**2 + vec_y**2 > MAX_TARGET_LOCK_DISTANCE_SQ:
                ...
                player_state['angulo'] = (-math.degrees(math.atan2(vec_y, vec_x)) - 90) % 360
        else:
            player_state['alvo_lock'] = None
    
    elif player_state['alvo_mouse']:
        vec_x = player_state['alvo_mouse'][0] - player_state['x']
        vec_y = player_state['alvo_mouse'][1] - player_state['y']
        if vec_x**2 + vec_y**2 > 25:
            player_state['angulo'] = (-math.degrees(math.atan2(vec_y, vec_x)) - 90) % 360
    
    # Rotação manual
    if not player_state['alvo_lock'] and not player_state['alvo_mouse']:
        if player_state['teclas']['a']:
            player_state['angulo'] = (player_state['angulo'] + (5 * dt)) % 360
        if player_state['teclas']['d']:
            player_state['angulo'] = (player_state['angulo'] - (5 * dt)) % 360
    ...
    if player_state['alvo_mouse'] and not (player_state['teclas']['w'] or player_state['teclas']['s']):
        ...
        if dist > vel:
            vx, vy = (dx / dist) * vel, (dy / dist) * vel
        else:
            player_state['x'], player_state['y'] = tx, ty
            player_state['alvo_mouse'] = None
            vx, vy = 0, 0
    
    # Aplica movimento com limites do mapa
    ...
    if pode_atirar and cooldown_ok:
        ...
        vel_x = -math.sin(rad) * velocidade_final
        vel_y = -math. cos(rad) * velocidade_final
        ...
        proj = {
            ...
            'acerto_garantido': vai_acertar,
            'velocidade_real': velocidade_final,
            'vel_x': vel_x,
            'vel_y': vel_y
        }
        
        if player_state['alvo_lock']:
            proj['tipo_proj'] = 'teleguiado'
            proj['velocidade'] = velocidade_final
            proj['alvo_id'] = player_state['alvo_lock']
        else:
            proj['tipo_proj'] = 'normal'
            proj['velocidade'] = 25
            proj['vel_x'] = -math.sin(rad) * 25
            proj['vel_y'] = -math. cos(rad) * 25
...
def process_auxiliaries_logic(p, living_targets, agora_ms):
    """Processa a lógica dos auxiliares (drones)"""
    new_projs = []
    
    nivel_aux = p. get('nivel_aux', 0)
    if nivel_aux <= 0 or not p.get('alvo_lock'):
        return new_projs
    
    t_id = p['alvo_lock']
    target = None
    for x in living_targets:
        if x. get('id', x.get('nome')) == t_id and x.get('hp', 0) > 0:
            target = x
            break
    ...
    # Garante que aux_cooldowns existe e tem tamanho suficiente
    if 'aux_cooldowns' not in p:
        p['aux_cooldowns'] = [0] * 4
    while len(p['aux_cooldowns']) < nivel_aux:
        p['aux_cooldowns'].append(0)
    ...
    for i in range(nivel_aux):
        ...
        dir_x = (target['x'] - ax) / dist
        dir_y = (target['y'] - ay) / dist
        
        tipo_aux = 'player_pvp' if p.get('is_pvp') else 'player_pve'
        if p['nivel_dano'] >= s. MAX_NIVEL_DANO:
            tipo_aux += '_max'
        
        vai_acertar = random.random() < s.CHANCE_ACERTO_TIRO
        velocidade_final = 14 * (1.5 if vai_acertar else 1.0)
        ...
        proj = {
            ...
            'alvo_id': t_id,
            'timestamp_criacao': agora_ms,
            'vel_x': dir_x * velocidade_final,
            'vel_y': dir_y * velocidade_final,
            'acerto_garantido': vai_acertar,
            'velocidade_real': velocidade_final
        }
...
# ==================================================================================
# 5. LÓGICA DE PROJÉTEIS
# ==================================================================================

def update_projectile_physics(proj, all_targets, agora_ms, dt=1.0):
    """Atualiza a física dos projéteis"""
    
    # Projétil teleguiado que vai errar
    ...
    # Projétil teleguiado normal
    if proj. get('tipo_proj') == 'teleguiado' and proj.get('alvo_id'):
        target = None
        for t in all_targets:
            if t. get('id', t.get('nome')) == proj['alvo_id'] and t.get('hp', 0) > 0:
                target = t
                break
        
        if target:
            target_pos = Vector2(target['x'], target['y'])
            curr_pos = Vector2(proj['x'], proj['y'])
            desired_vec = target_pos - curr_pos
            dist = desired_vec. length()
            
            velocidade = proj.get('velocidade_real', proj.get('velocidade', VELOCIDADE_PROJETIL_TELE))
            passo_neste_frame = velocidade * dt
            
            # Se está muito perto, vai direto ao alvo
            if dist <= passo_neste_frame:
                proj['x'] = target['x']
                proj['y'] = target['y']
                return
            ...
            if dist > 0:
                ...
                new_vel = desired_dir * velocidade
                proj['vel_x'] = new_vel.x
                proj['vel_y'] = new_vel.y
                proj['x'] += new_vel.x * dt
                proj['y'] += new_vel.y * dt
        else:
            # Alvo morreu, continua em linha reta
            proj['tipo_proj'] = 'normal'
            proj['x'] += proj.get('vel_x', 0) * dt
            proj['y'] += proj.get('vel_y', 0) * dt
    else:
...
def update_npc_generic_logic(npc, players_dict, agora_ms, dt=1.0):
    """Atualiza a lógica de NPCs genéricos"""
    if npc.get('hp', 0) <= 0:
        return None
    ...
    dist = math.sqrt(dist_min_sq) if dist_min_sq > 0 else 1
    ...
    npc['angulo'] = (-math.degrees(radianos) - 90) % 360
    
    # Bomba explode ao chegar perto
    if tipo == 'bomba':
    ...
    if dist_min_sq < DISTANCIA_TIRO_PERSEGUIDOR_SQ:
        ...
        if agora_ms - npc. get('ultimo_tiro_tempo', 0) > cooldown:
            ...
            if tipo == 'tiro_rapido':
                ...
                # Encontra ID do alvo mais próximo
                dist_min_alvo = float('inf')
                for p_state in players_dict. values():
                    if p_state. get('hp', 0) <= 0:
                        continue
                    p_dist_sq = (npc['x'] - p_state['x'])**2 + (npc['y'] - p_state['y'])**2
                    if p_dist_sq < dist_min_alvo:
                        dist_min_alvo = p_dist_sq
                        alvo_id_proj = p_state['nome']
            
            vai_acertar = random.random() < s.CHANCE_ACERTO_TIRO
            angulo_tiro = radianos
            if not vai_acertar:
                angulo_tiro += random.uniform(-0.5, 0.5)
            
            if tipo_proj_npc == 'normal':
                vel_x = math.cos(angulo_tiro) * velocidade_proj
                vel_y = math.sin(angulo_tiro) * velocidade_proj
            else:
                if not alvo_id_proj:
                    return None
                if not vai_acertar:
                    ...
                    vel_x = dir_x * velocidade_proj
                    vel_y = dir_y * velocidade_proj
            ...
            return {
                ...
                'pos_inicial_x': npc['x'],
                'pos_inicial_y': npc['y'],
                'angulo_rad': angulo_tiro,
                'velocidade': velocidade_proj,
                'dano': 1,
                'tipo': 'npc',
                'tipo_proj': tipo_proj_npc,
                'vel_x': vel_x,
                'vel_y': vel_y,
                'alvo_id': alvo_id_proj,
                'timestamp_criacao': agora_ms,
                'acerto_garantido': vai_acertar
            }
...
def update_mothership_logic(npc, players_dict, agora_ms, room_ref, dt=1.0):
    """Atualiza a lógica do Mothership"""
    if npc.get('hp', 0) <= 0:
        return None
    
    target_id = npc.get('ia_alvo_retaliacao')
    target = None
    
    if target_id:
        for p in players_dict.values():
            if p.get('nome') == target_id and p.get('hp', 0) > 0:
                target = p
                break
        if not target:
            npc['ia_alvo_retaliacao'] = None
            npc['ia_estado'] = 'VAGANDO'
    ...
    if target:
        ...
        if dist > 700:
            npc['x'] += (dx / dist) * velocidade
            npc['y'] += (dy / dist) * velocidade
        elif dist < 500:
            npc['x'] -= (dx / dist) * velocidade
            npc['y'] -= (dy / dist) * velocidade
        
        # Spawn minions
        minions_vivos = [m for m in room_ref.npcs 
                         if m.get('tipo') == 'minion_mothership' 
                         and m.get('owner_id') == npc['id']
                         and m.get('hp', 0) > 0]
        
        if len(minions_vivos) < MAX_MINIONS_MOTHERSHIP:
            if agora_ms - npc.get('ia_ultimo_spawn', 0) > 2000:
                npc['ia_ultimo_spawn'] = agora_ms
                m = server_spawnar_minion_mothership(
                    npc, target_id, len(minions_vivos), 
                    MAX_MINIONS_MOTHERSHIP, room_ref. next_npc_id
                )
                room_ref.npcs.append(m)
                room_ref.next_npc_id += 1
    else:
        npc['ia_estado'] = 'VAGANDO'
        
        wander_target = npc.get('ia_wander_target')
        if wander_target:
            wx, wy = wander_target
            d_sq = (wx - npc['x'])**2 + (wy - npc['y'])**2
            if d_sq < 100**2:
                npc['ia_wander_target'] = None
            else:
                d = math.sqrt(d_sq)
                npc['x'] += ((wx - npc['x']) / d) * 0.5 * dt
                npc['y'] += ((wy - npc['y']) / d) * 0.5 * dt
        else:
            npc['ia_wander_target'] = (
                random.randint(100, s.MAP_WIDTH - 100),
                random. randint(100, s.MAP_HEIGHT - 100)
            )
...
def update_boss_congelante_logic(npc, players_dict, agora_ms, room_ref, dt=1.0):
    """Atualiza a lógica do Boss Congelante"""
    if npc.get('hp', 0) <= 0:
        return None
    ...
    if target:
        ...
        if agora_ms - npc.get('ia_ultimo_spawn_minion', 0) > COOLDOWN_SPAWN_MINION_CONGELANTE:
            minions = [m for m in room_ref.npcs 
                       if m.get('tipo') == 'minion_congelante' 
                       and m.get('owner_id') == npc['id']
                       and m. get('hp', 0) > 0]
            
            if len(minions) < MAX_MINIONS_CONGELANTE:
                npc['ia_ultimo_spawn_minion'] = agora_ms
                m = server_spawnar_minion_congelante(
                    npc, target['nome'], len(minions),
                    MAX_MINIONS_CONGELANTE, room_ref.next_npc_id
                )
                room_ref.npcs.append(m)
                room_ref.next_npc_id += 1
        ...
        if agora_ms - npc.get('ultimo_tiro_tempo', 0) > cooldown_tiro:
            ...
            vel_x = (dx / dist) * VELOCIDADE_PROJ_CONGELANTE
            vel_y = (dy / dist) * VELOCIDADE_PROJ_CONGELANTE
            ...
            return {
                ...
                'dano': 1,
                'tipo': 'npc',
                'tipo_proj': 'congelante',
                'vel_x': vel_x,
                'vel_y': vel_y,
                'velocidade': VELOCIDADE_PROJ_CONGELANTE,
                'timestamp_criacao': agora_ms
            }
    else:
        # Wander
        wander = npc.get('ia_wander_target')
        if not wander or (npc['x'] - wander[0])**2 + (npc['y'] - wander[1])**2 < 100**2:
            npc['ia_wander_target'] = (
                random.randint(100, s.MAP_WIDTH - 100),
                random. randint(100, s.MAP_HEIGHT - 100)
            )
...
def update_minion_logic(npc, players_dict, agora_ms, room_ref, dt=1.0):
    """Atualiza a lógica dos Minions"""
    if npc.get('hp', 0) <= 0:
        return None
    
    # Verifica se o dono ainda está vivo
    owner = None
    for n in room_ref.npcs:
        if n.get('id') == npc. get('owner_id') and n.get('hp', 0) > 0:
            owner = n
            break
    ...
    # Encontra alvo do dono
    target = None
    target_id = owner.get('ia_alvo_retaliacao')
    if target_id:
        for p in players_dict.values():
            if p. get('nome') == target_id and p.get('hp', 0) > 0:
                target = p
    ...
    # Se é minion congelante e tem alvo próximo, vai atrás
    if target and npc.get('tipo') == 'minion_congelante':
        ...
        if d_owner_target < MINION_CONGELANTE_LEASH_RANGE**2:
            ...
            if d_to_target_sq > 150**2:
                dest_x, dest_y = target['x'], target['y']
    
    # Movimento suave
    fator_interp = 0.1 * dt
    npc['x'] += (dest_x - npc['x']) * fator_interp
    npc['y'] += (dest_y - npc['y']) * fator_interp
...
def server_spawnar_inimigo_aleatorio(x, y, npc_id):
    ...
    if chance < 0.05:
        tipo, hp, tamanho, cooldown_tiro, pontos = "bomba", 1, 25, 999999, 3
    elif chance < 0.10:
        tipo, hp, tamanho, cooldown_tiro, pontos = "tiro_rapido", 10, 30, 1500, 20
    elif chance < 0.15:
        tipo, hp, tamanho, cooldown_tiro, pontos = "atordoador", 5, 30, 5000, 25
    elif chance < 0.35:
        tipo, hp, tamanho, cooldown_tiro, pontos = "atirador_rapido", 1, 30, 500, 10
    elif chance < 0.55:
        tipo, hp, tamanho, cooldown_tiro, pontos = "rapido", 5, 30, 800, 9
    else:
        tipo, hp, tamanho, cooldown_tiro, pontos = "perseguidor", 3, 30, COOLDOWN_TIRO_PERSEGUIDOR, 5
...
def server_spawnar_boss_congelante(x, y, npc_id):
    """Spawna um Boss Congelante"""
    hp_boss = getattr(s, 'HP_BOSS_CONGELANTE', 150)
    tamanho_boss = getattr(s, 'TAMANHO_BOSS_CONGELANTE', 70)
...
def server_spawnar_minion_mothership(owner, target_id, index, max_minions, npc_id_num):
    """Spawna um Minion do Mothership"""
    angulo_inicial = (360 / max(max_minions, 1)) * index
    
    return {
        'id': f"minion_ms_{npc_id_num}",
        'tipo': 'minion_mothership',
        'owner_id': owner['id'],
        'x': owner['x'],
        'y': owner['y'],
        'angulo': 0.0,
        ...
        'ultimo_tiro_tempo': 0,
        'pontos_por_morte': 3,
        'ia_angulo_orbita': angulo_inicial,
        'ia_vel_orbita': 2,
        'ia_raio_orbita': 100
    }
...
def server_spawnar_minion_congelante(owner, target_id, index, max_minions, npc_id_num):
    """Spawna um Minion do Boss Congelante"""
    angulo_inicial = (360 / max(max_minions, 1)) * index
    
    return {
        'id': f"minion_bc_{npc_id_num}",
        'tipo': 'minion_congelante',
        'owner_id': owner['id'],
        'x': owner['x'],
        'y': owner['y'],
        'angulo': 0.0,
        'hp': HP_MINION_CONGELANTE,
        'max_hp': HP_MINION_CONGELANTE,
        'tamanho': 25,
        'cooldown_tiro': COOLDOWN_TIRO_MINION_CONGELANTE,
        'ultimo_tiro_tempo': 0,
...
def server_spawnar_obstaculo(pos_referencia_lista, map_width, map_height, npc_id):
    """Spawna um obstáculo destrutível"""
    x, y = server_calcular_posicao_spawn(pos_referencia_lista, map_width, map_height)
    
    raio_min = getattr(s, 'OBSTACULO_RAIO_MIN', 20)
    raio_max = getattr(s, 'OBSTACULO_RAIO_MAX', 50)
    pontos_min = getattr(s, 'OBSTACULO_PONTOS_MIN', 1)
    pontos_max = getattr(s, 'OBSTACULO_PONTOS_MAX', 5)
    
    raio = random.randint(raio_min, raio_max)
    raio_norm = max(raio_min, min(raio, raio_max))
    
    # Calcula pontos baseado no tamanho
    range_r = raio_max - raio_min
    range_p = pontos_max - pontos_min
    pct = (raio_norm - raio_min) / range_r if range_r > 0 else 0
    pts = int(round(pontos_min + (pct * range_p)))
...
# ==================================================================================
# 9. FUNÇÕES AUXILIARES (UNUSED/PLACEHOLDER)
# ==================================================================================

def server_processar_explosao_kamikaze(npc, players_dict, agora_ms):
    """Processa explosão de NPC kamikaze (placeholder)"""
    pass