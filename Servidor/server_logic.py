...
# ==================================================================================
# 2. FUNÇÕES LÓGICAS (COM PROTEÇÃO DE LOOP INFINITO)
# ==================================================================================
...
def server_ganhar_pontos(player_state, quantidade):
    ...
    safety_break = 0
    if player_state['_limiar_pontos_atual'] <= 0:
        player_state['_limiar_pontos_atual'] = 100 
...
def server_comprar_upgrade(player_state, tipo_upgrade):
    ...
    if tipo_upgrade == "motor" and player_state['nivel_motor'] < MAX_NIVEL_MOTOR:
        if pode_comprar(custo_padrao): player_state['pontos_upgrade_disponiveis'] -= custo_padrao; player_state['total_upgrades_feitos'] += custo_padrao; player_state['nivel_motor'] += 1
    elif tipo_upgrade == "dano" and player_state['nivel_dano'] < MAX_NIVEL_DANO:
        if pode_comprar(custo_padrao): player_state['pontos_upgrade_disponiveis'] -= custo_padrao; player_state['total_upgrades_feitos'] += custo_padrao; player_state['nivel_dano'] += 1
        ...
        if num < MAX_AUXILIARES:
            ...
            if player_state['pontos_upgrade_disponiveis'] >= c_aux:
                if pode_comprar(c_aux): player_state['pontos_upgrade_disponiveis'] -= c_aux; player_state['total_upgrades_feitos'] += c_aux; player_state['nivel_aux'] += 1
    elif tipo_upgrade == "max_health" and player_state['nivel_max_vida'] < len(s.VIDA_POR_NIVEL) - 1:
        if pode_comprar(custo_padrao): player_state['pontos_upgrade_disponiveis'] -= custo_padrao; player_state['total_upgrades_feitos'] += custo_padrao; player_state['nivel_max_vida'] += 1; player_state['max_hp'] = s.VIDA_POR_NIVEL[player_state['nivel_max_vida']]; player_state['hp'] += 1
    elif tipo_upgrade == "escudo" and player_state['nivel_escudo'] < MAX_NIVEL_ESCUDO:
        if pode_comprar(custo_padrao): player_state['pontos_upgrade_disponiveis'] -= custo_padrao; player_state['total_upgrades_feitos'] += custo_padrao; player_state['nivel_escudo'] += 1
    ...
    for _ in range(20):
        ...
        if pos_referencia_lista: 
            for px, py in pos_referencia_lista:
                if (x - px)**2 + (y - py)**2 < 600**2: longe = False; break
...
def update_player_logic(player_state, lista_alvos_busca, agora_ms, map_width, map_height, dt=1.0): 
    ...
    if player_state.get('is_pre_match', False): player_state['alvo_mouse'] = None; player_state['teclas'] = {'w':False,'a':False,'s':False,'d':False,'space':False}; return None 
    ...
    if (player_state['teclas']['space'] or player_state['alvo_lock']) and \
       (agora_ms - player_state['ultimo_tiro_tempo'] > player_state['cooldown_tiro']):
        ...
        velocidade_final = VELOCIDADE_PROJETIL_TELE
        if vai_acertar:
            velocidade_final *= 1.5 

        angulo_disparo = rad

        vel_x = -math.sin(angulo_disparo) * velocidade_final
        vel_y = -math.cos(angulo_disparo) * velocidade_final
        ...
        if player_state['alvo_lock']:
            proj.update({'tipo_proj': 'teleguiado', 'velocidade': velocidade_final, 
                         'alvo_id': player_state['alvo_lock'], 
                         'vel_x': vel_x, 'vel_y': vel_y})
        else:
            proj.update({'tipo_proj': 'normal', 'velocidade': 25, 
                         'vel_x': -math.sin(angulo_disparo)*25, 'vel_y': -math.cos(angulo_disparo)*25})
        return proj
    return None
            ...
            for i in range(p['nivel_aux']):
                ...
                if agora_ms > p['aux_cooldowns'][i]:
                    ...
                    if dist_sq < AUX_DISTANCIA_TIRO_SQ:
                        ...
                        proj = {
                            'id': f"{p['nome']}_aux{i}_{agora_ms}", 'owner_nome': p['nome'], 
                            'x': ax, 'y': ay, 'pos_inicial_x': ax, 'pos_inicial_y': ay, 
                            'dano': s.DANO_POR_NIVEL[p['nivel_dano']], 'tipo': tipo_aux, 
                            'tipo_proj': 'teleguiado', 'velocidade': velocidade_final, 'alvo_id': t_id, 
                            'timestamp_criacao': agora_ms, 'vel_x': dir_x * velocidade_final, 'vel_y': dir_y * velocidade_final,
                            'acerto_garantido': vai_acertar,
                            'velocidade_real': velocidade_final
                        }
...
def update_projectile_physics(proj, all_targets, agora_ms, dt=1.0):
    ...
    if proj.get('tipo_proj') == 'teleguiado' and proj.get('alvo_id'):
        ...
        if target and target.get('hp', 0) > 0:
            ...
            velocidade = proj.get('velocidade_real', proj['velocidade'])
...
def update_npc_generic_logic(npc, players_dict, agora_ms, dt=1.0):
    ...
    npc['angulo'] = -math.degrees(radianos) - 90; npc['angulo'] %= 360
    
    if npc['tipo'] == 'bomba':
        if dist_min_sq <= KAMIKAZE_DIST_DETONACAO_SQ: npc['hp'] = 0; return None
        return None 
    ...
    if dist_min_sq < DISTANCIA_TIRO_PERSEGUIDOR_SQ: 
        if agora_ms - npc['ultimo_tiro_tempo'] > npc['cooldown_tiro']:
            ...
            if tipo_proj_npc == 'normal':
                vel_x = math.cos(angulo_tiro) * velocidade_proj; vel_y = math.sin(angulo_tiro) * velocidade_proj
            else: 
            ...
            return {'id': f"{npc['id']}_{agora_ms}", 'owner_nome': npc['id'], 'x': npc['x'], 'y': npc['y'], 'pos_inicial_x': npc['x'], 'pos_inicial_y': npc['y'], 'angulo_rad': angulo_tiro, 'velocidade': velocidade_proj, 'dano': 1, 'tipo': 'npc', 'tipo_proj': tipo_proj_npc, 'alvo_id': alvo_id_proj, 'timestamp_criacao': agora_ms, 'vel_x': vel_x, 'vel_y': vel_y, 'acerto_garantido': vai_acertar}
    return None
...
def update_mothership_logic(npc, players_dict, agora_ms, room_ref, dt=1.0):
    ...
    target = None
    if target_id:
        target = next((p for p in players_dict.values() if p['nome'] == target_id), None)
        if not target or target['hp'] <= 0:
            npc['ia_alvo_retaliacao'] = None; target = None; npc['ia_estado'] = 'VAGANDO'
    
    if target:
        npc['ia_estado'] = 'RETALIANDO'
        dist_sq = (target['x'] - npc['x'])**2 + (target['y'] - npc['y'])**2; dist_real = math.sqrt(dist_sq)
        velocidade = 1.0 * dt; dx, dy = target['x'] - npc['x'], target['y'] - npc['y']
        if dist_real > 700: npc['x'] += (dx / dist_real) * velocidade; npc['y'] += (dy / dist_real) * velocidade
        elif dist_real < 500: npc['x'] -= (dx / dist_real) * velocidade; npc['y'] -= (dy / dist_real) * velocidade
        ...
        if len(minions_vivos) < MAX_MINIONS_MOTHERSHIP:
            if agora_ms - npc.get('ia_ultimo_spawn', 0) > 2000:
                ...
                m = server_spawnar_minion_mothership(npc, target_id, len(minions_vivos), MAX_MINIONS_MOTHERSHIP, room_ref.next_npc_id)
...
def update_boss_congelante_logic(npc, players_dict, agora_ms, room_ref, dt=1.0):
    ...
    if target_id:
        target = next((p for p in players_dict.values() if p['nome'] == target_id), None)
        if not target or target['hp'] <= 0: npc['ia_alvo_retaliacao'] = None; target = None
    
    if target:
        velocidade = 1.8 * dt; dx, dy = target['x'] - npc['x'], target['y'] - npc['y']; dist = math.sqrt(dx**2 + dy**2) + 0.1
        npc['x'] += (dx/dist) * velocidade; npc['y'] += (dy/dist) * velocidade
        ...
        if agora_ms - npc.get('ultimo_tiro_tempo', 0) > s.COOLDOWN_TIRO_CONGELANTE:
            npc['ultimo_tiro_tempo'] = agora_ms
            vel_x = (dx/dist) * VELOCIDADE_PROJ_CONGELANTE; vel_y = (dy/dist) * VELOCIDADE_PROJ_CONGELANTE
...
def update_minion_logic(npc, players_dict, agora_ms, room_ref, dt=1.0):
    ...
    target = None
    if owner.get('ia_alvo_retaliacao'): target = next((p for p in players_dict.values() if p['nome'] == owner['ia_alvo_retaliacao']), None)
    ...
    fator_interp = 0.1 * dt
    npc['x'] += (dest_x - npc['x']) * fator_interp; npc['y'] += (dest_y - npc['y']) * fator_interp
    
    if target and agora_ms - npc.get('ultimo_tiro_tempo', 0) > npc['cooldown_tiro']:
         d_target = (target['x']-npc['x'])**2 + (target['y']-npc['y'])**2
         if d_target < 400**2:
             npc['ultimo_tiro_tempo'] = agora_ms
             dx, dy = target['x'] - npc['x'], target['y'] - npc['y']; dist = math.sqrt(dx**2 + dy**2) + 0.1; rad_tiro = math.atan2(dy, dx)
